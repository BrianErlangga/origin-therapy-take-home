...
**1.5. Grounding gate** (`verifyGrounding`, in `src/agent.ts`). Between extract and route, a deterministic check confirms each high-cost field — `child_name`, `dob_or_age`, `member_id`, `parent_contact`, `payer` — is traceable to the source text the model read (sender + subject + body). The selection principle: ground the fields where a hallucination causes a *silent, costly, slow-to-surface* failure — a claim submitted to the wrong carrier or member ID that denies months later, a child misidentified, or PHI drafted to the wrong contact. These are also the fields the router acts on with real tools (`verify_insurance(payer, member_id)`, `hold_slot(patient_ref)`, `search_patient`, `draft_message(contact)`), and notably `verify_insurance` trusts whatever payer/ID it is handed — so grounding is the only thing standing between a hallucinated carrier and a denied claim. `discipline` (an enum staff confirms) and `diagnosis_or_concern` (a free-text summary) are left out as lower-stakes and operationally visible. No second LLM call — re-asking the model that just hallucinated rarely changes its answer; deterministic checks are reproducible, free, and themselves auditable. If a field fails, the agent retries extraction once with a verbatim-copy instruction naming the failed fields; if it still fails, it fails closed (see Guardrails).
...
- **Grounded extraction (fail-closed).** Before the router acts on a high-cost field (`member_id`, `payer`, `child_name`, `dob_or_age`, `parent_contact`), a deterministic check confirms the value traces to the source — so a hallucinated carrier, ID, or contact can't silently enter a record and surface months later as a denied claim. Each field has a format-tolerant check (member IDs ignore dashes, phones compare digits, payers and names match per significant token, dates match by numeric component or verbatim phrase); ages must appear in an age context so a stray number elsewhere can't satisfy them. On failure the agent retries extraction once with a verbatim-copy instruction; if it still fails it fails closed — no tools fire, the field is listed in `missing_info`, and the reason is recorded in `decision_rationale`. The asymmetry is deliberate: a false alarm costs a few minutes of staff review, but acting on a fabricated value corrupts a patient record. **Safeguarding is the exception** — it always escalates even when a field is ungrounded, since that path uses only the item ID (not the identity fields) and withholding a P0 alert over an extraction problem would be the worse error. Field checks and edge cases are unit-tested in `src/grounding.test.ts`.
...
- **Grounding beyond field provenance.** The grounding gate verifies that an extracted value came from the source text — not that it is *correct for this patient*. Boundaries deliberately out of scope: (1) **Association** — if a message mentions a sibling, a `member_id` can be perfectly grounded yet belong to the wrong child; grounding confirms the value is in the document, not that it was matched to the right person. (2) **Source correctness** — if the referring provider wrote the wrong ID on the fax, we faithfully ground a wrong value. (3) **Acronym ↔ expansion** — a payer written as `BCBS` but extracted as `Blue Cross Blue Shield` (or vice versa) shares no token and would flag (a false positive the verbatim retry usually rescues, since the classifier is told to extract verbatim); likewise a word source normalized to a different digit form (`"seven"` → `"7"`). A day/month transposition (`09/03` vs `03/09`) is intentionally *not* flagged, since both readings are equally supported by an ambiguous source. The next steps here are entity-level association checks and grounding the additional claim-critical fields a real EHR exposes (prior-auth numbers, CPT/ICD codes, rendering-provider NPI), which this schema does not include.
